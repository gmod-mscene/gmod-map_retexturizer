...
function Panels:SetGeneral(parent, frameType, info)
	local frame = MR.CL.Panels:StartContainer("gui.mr.general.name", parent, frameType, info)
	...
	local changeAll = vgui.Create("DButton", panel)
		...
		changeAll:SetText("gui.mr.general.all")
		...
		changeAll.DoClick = function()
			...
			local text = vgui.Create("DLabel", qPanel)
				text:SetPos(10, 25)
				text:SetSize(300, 25)
				text:SetText("gui.mr.general.allPrompt")
			...
			local buttonYes = vgui.Create("DButton", qPanel)
				...
				buttonYes.DoClick = function()
					net.Start("SV.Materials:ApplyAll")
					net.SendToServer()
					qPanel:Close()
				end
	...
	local browser = vgui.Create("DButton", panel)
		...
		browser:SetText("gui.mr.general.browser")
	...
	local preview = vgui.Create("DCheckBoxLabel", panel)
		...
		preview:SetText("gui.mr.general.preview")
	...
	local decalsMode = vgui.Create("DCheckBoxLabel", panel)
		...
		decalsMode:SetText("gui.mr.general.decal")
