...
function Browser:Create()
	...
	if not Browser:GetSelf() then
		...
		local window = vgui.Create("DFrame")
			...
			window:SetTitle("gui.mr.mb.name")
		...
		local leftButton = vgui.Create("DButton", window)
			...
			leftButton:SetText("gui.mr.mb.preview")
		...
		local middleButton = vgui.Create("DButton", window)
			...
			middleButton:SetText("gui.mr.mb.copy")
		...
		local rightButton = vgui.Create("DButton", window)
			...
			rightButton:SetText("gui.mr.mb.tool")
...
function Browser:Create_PopulateLists(TreeList, Scroll, browserPreviewMaterial)
	...
	local node = TreeList:AddNode("gui.mr.mb.mats")
				...
				local icon = vgui.Create("DImageButton", iconBackground)
					...
					icon.OnDepressed = function()
						...
						elseif input.IsMouseDown(108) then
							...
							Browser:ReadDir_PrintOverlayMessage(iconBackground, maxSize, pressed, arq, 14, "gui.mr.mb.tool")
...
function Browser:ReadDir_CreateResetIconsPanel(Scroll, setOnly, n, dir, fdir, ext, browserPreviewMaterial)
	...
	if dir..fdir == "materials/decals" or
		...
		Scroll.IconsList.warning:SetText("gui.mr.mb.block")
